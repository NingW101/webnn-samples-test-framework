...
const chromeCanaryPath =
    `${process.env.LOCALAPPDATA}/Google/Chrome SxS/Application/`;
const dllFileName = 'DirectML.dll';
...
function getVersionFolder(basePath) {
  return fs.readdirSync(basePath, {withFileTypes: true})
             .find(e => e.isDirectory() && /^\d+\.\d+\.\d+\.\d+$/.test(e.name))
             ?.name ||
      null;
}

const dllPath = path.join(chromeCanaryPath, dllFileName);

if (!fs.existsSync(dllPath)) {
  log(`Error: ${dllFileName} not found in ${chromeCanaryPath}`);
  process.exit(1);
}

const versionFolder = getVersionFolder(chromeCanaryPath);
if (!versionFolder) {
  log('Error: No versioned folder found in Chrome Canary application directory.');
  process.exit(1);
}

const destPath = path.join(chromeCanaryPath, versionFolder, dllFileName);
fs.copyFileSync(dllPath, destPath);
log(`${dllFileName} copied to ${destPath}`);

const child = spawn(
    'git stash && git pull && git stash pop && npm i && npm run test',
    {shell: true});
